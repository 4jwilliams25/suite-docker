...
function confirmDialog(arrayContents, formName) {
  ...
  for (var key in data) {
    ...
    if (
      opener.forms[formName] &&
      opener.getElementById(key + "_label") != null &&
      !key.match(/account/)
    ) {
      var dataLabel = opener
        .getElementById(key + "_label")
        .innerHTML.replace(/\n/gi, "")
        .replace(/<\/?[^>]+(>|$)/g, "");
      labels += dataLabel + " \n";
      ...
      if (window.opener.document.forms[formName].elements[key]) {
        oldData +=
          dataLabel + " " + opener.forms[formName].elements[key].value + "\n";
      }
    }
  }
  var popupConfirm = 0;
...
function send_back(module, id) {
  ...
  let fill_array_contents = function (the_key, the_name) {
    ...
    if (module !== "" && id !== "") {
      if (
        associated_row_data["DOCUMENT_NAME"] &&
        the_key.toUpperCase() === "NAME"
      ) {
        the_value = associated_row_data["DOCUMENT_NAME"];
      } else if (
        (the_key.toUpperCase() === "USER_NAME" ||
          the_key.toUpperCase() === "LAST_NAME" ||
          the_key.toUpperCase() === "FIRST_NAME") &&
        typeof is_show_fullname != "undefined" &&
        is_show_fullname &&
...
function send_back_teams(
  ...
  let passthru_data = Object();
  if (
    typeof request_data.call_back_function == "undefined" &&
...
function send_back_selected(module, form, field, error_message, request_data) {
  ...
  SUGAR.util.globalEval(
    "var selection_list_array = {" + array_contents.join(",") + "}"
  );
  let form_request_data =
    window.document.forms["popup_query_form"].request_data.value;
  if (
    /^[\],:{}\s]*$/.test(
      form_request_data
...
function toggleMore(spanId, img_id, module, action, params) {
  toggle_more_go = function () {
    ...
    success = function (data) {
      ...
      return oReturn(
        SUGAR.util.additionalDetailsCache[spanId]["body"],
        SUGAR.util.additionalDetailsCache[spanId]["caption"],
        SUGAR.util.additionalDetailsCache[spanId]["width"],
        SUGAR.util.additionalDetailsCache[spanId]["theme"]
      );
    };
    ...
    if (typeof SUGAR.util.additionalDetailsCalls[spanId] != "undefined") return;
    ajaxStatus.showStatus(SUGAR.language.get("app_strings", "LBL_LOADING"));
    url = "index.php?module=" + module + "&action=" + action + "&" + params;
    SUGAR.util.additionalDetailsCalls[spanId] = YAHOO.util.Connect.asyncRequest(
      "GET",
      url,
  ...
  SUGAR.util.additionalDetailsRpcCall = window.setTimeout(
    "toggle_more_go()",
    250
  );
}
SUGAR.util.doWhen(
  "window.document.forms['popup_query_form'] != null " +
  ...
  function () {
    if (window.document.forms["popup_query_form"].request_data.value == "") {
      window.document.forms["popup_query_form"].request_data.value =
        window.opener.get_popup_request_data();
    }
  }
);
