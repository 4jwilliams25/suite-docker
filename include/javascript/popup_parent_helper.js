  ...
  if (request_data && request_data.field_to_name_array != "undefined") {
    for (var key in request_data.field_to_name_array) {
      if (key.toLowerCase() != "id") {
        field_to_name_array_url +=
          "&field_to_name[]=" + encodeURIComponent(key.toLowerCase());
      }
    }
  }
  ...
  for (var the_key in name_to_value_array) {
    if (the_key == "toJSON") {
      ...
      var displayValue = name_to_value_array[the_key]
        ...
        .replace(/&quot;/gi, '"');
      if (
        window.document.forms[form_name] &&
        window.document.forms[form_name].elements[the_key]
      ) {
        window.document.forms[form_name].elements[the_key].value = displayValue;
        SUGAR.util.callOnChangeListers(
          window.document.forms[form_name].elements[the_key]
        );
      }
    }
...
function set_return_and_save(popup_reply_data) {
  ...
  for (var the_key in name_to_value_array) {
    if (the_key == "toJSON") {
      ...
      window.document.forms[form_name].elements[the_key].value =
        name_to_value_array[the_key];
      SUGAR.util.callOnChangeListers(
        window.document.forms[form_name].elements[the_key]
      );
    }
  }
  window.document.forms[form_name].return_module.value =
    window.document.forms[form_name].module.value;
  window.document.forms[form_name].return_action.value = "DetailView";
  window.document.forms[form_name].return_id.value =
    window.document.forms[form_name].record.value;
...
function set_return_and_save_targetlist(popup_reply_data) {
  ...
  for (var the_key in name_to_value_array) {
    if (the_key == "toJSON") {
      ...
      window.document.forms[form_index].elements[
        get_element_index(form_index, the_key)
      ].value = name_to_value_array[the_key];
      SUGAR.util.callOnChangeListers(
        window.document.forms[form_index].elements[
          get_element_index(form_index, the_key)
        ]
  ...
  window.document.forms[form_index].elements[
    get_element_index(form_index, "return_module")
  ].value =
    window.document.forms[form_index].elements[
      get_element_index(form_index, "module")
    ].value;
  window.document.forms[form_index].elements[
    get_element_index(form_index, "return_action")
  ].value = "ListView";
  window.document.forms[form_index].elements[
    get_element_index(form_index, "uids")
  ].value = uids;
...
function get_initial_filter_by_account(form_name) {
  ...
  var account_name = escape(
    window.document.forms[form_name].account_name.value
  );
  var initial_filter =
